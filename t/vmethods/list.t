...
# add some new list ops in various different ways
my $tt = Template::TT2->new;

$Template::TT2::Stash::LIST_OPS->{ sum } = \&sum;
Template::TT2::Stash->define_vmethod( list => odd => \&odd );

$tt->context->define_vmethod( 
    list => jumble => \&jumble 
);
$tt->context->define_vmethods(
    list => {
        oddnos   => \&odd,
        jumblate => \&jumble,
    }
...
# 'array' is an alias for 'list'
$tt->context->define_vmethod(array  => jumbloid => \&jumble);
...
-- test list.join with joint --
[% metavars.join(', ') %]
-- expect --
foo, bar, baz, qux, wiz, waz, woz

-- test list.join with another joint --
[% metavars.sort.join(', ') %]
-- expect --
...
-- test sort list of objects by key --
...
-- test sort list of objects by multi keys --
...
-- test sort list of numbers by key --
...
-- test the last list.splice of all --
[% list_one = [ 'a' 'b' 'c' 'd' 'e' ] -%]
...
-- test ha ha only joking, here's the scrabble splice --
...
-- test jumbloid method --
[% items = [0..6] -%]
[% items.jumbloid.join(', ') %]
[% items.jumbloid(3).join(', ') %]
-- expect --
1, 2, 3, 4, 5, 6, 0
4, 5, 6, 0, 1, 2, 3
