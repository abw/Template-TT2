package Template::TT2::Plugin::Date;
...
use Template::TT2::Class
    ...
    messages   => {
        bad_format => "Invalid time/date string: %s (should be 'h:m:s d:m:y')",
    };
...
our $DELEGATES     = {
    calc  => ['Date::Calc',  'Template::TT2::Plugin::Date::Calc'],
    manip => ['Date::Manip', 'Template::TT2::Plugin::Date::Manip'],
};


# generate calc() and manip() methods from the $DELEGATES table

class->methods(
    map {
        my $deleg = $DELEGATES->{ $_ };
        $_ => sub {
            my $self  = shift;
            class( $deleg->[0] )->load;
            return $deleg->[1]->new(NO_CONTEXT);
        }
    }
    keys %$DELEGATES
);
...
sub format {
    my $self   = shift;
    my $params = ref($_[LAST]) eq HASH ? pop(@_) : { };
    my $time   = @_ ? shift : ($params->{ time   } || $self->{ time   } || $self->now);
    my $format = @_ ? shift : ($params->{ format } || $self->{ format } || $FORMAT);
    my $locale = @_ ? shift : ($params->{ locale } || $self->{ locale });
    my $gmt    = @_ ? shift : ($params->{ gmt    } || $self->{ gmt    });
    my (@date, $datestr);
    ...
    if ($time =~ /^-?\d+$/) {
        ...
        return $self->error_msg( bad_format => $time )
            unless @date;
...
package Template::TT2::Plugin::Date::Calc;
