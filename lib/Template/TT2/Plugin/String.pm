package Template::TT2::Plugin::String;

use Template::TT2::Class
    version     => 0.01,
    debug       => 0,
    base        => 'Template::TT2::Plugin',
    constants   => 'LAST HASH ARRAY',
    throws      => 'String',
    as_text     => \&text,
    is_true     => 1;
...
sub new {
    ...
    $class->debug("Creating String from text: $text") if DEBUG;
...
sub output_filter {
    ...
    while (@$filter) {
        ...
        $self->debug("adding output filter $name(@$args)") if DEBUG;
...
Template::TT2::Plugin::String - Object oriented interface for string manipulation
...
This plugin is a mess (more on a conceptual level than in terms of 
implementation).  There's a lot of duplication with text virtual methods
and I'm not convinced that a String plugin object has any real benefits
(although some things are more easily do-able when text is wrapped up in
an object - like automatic filtering, encoding, etc).

However, the purpose of Template::TT2 is to provide a legacy version, so
we will remain compatible with TT2, warts and all.  This plugin module
will be removed, replaced or refactored in TT3.
